...
struct JeffJSCompiler {
    ...
    static func createFunction(ctx: JeffJSContext,
        ...
        // 2c. (Method calls are decided by the parser: `obj.m()`, `obj[k]()` and
        // a parenthesised reference `(obj.m)()` get get_field2/get_array_el2 +
        // call_method there. The old bytecode-level rewrite of every
        // get_field that reached a call as the callee — transformMethodCalls —
        // also turned `(0, obj.m)()` and `(a || obj.m)()` into method calls, and
        // across a `||` whose left side was taken it emitted call_method with
